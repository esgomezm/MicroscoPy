...
def upsample(
    x, out_channels=16, method="Upsampling2D", upsampling_factor=2, input_shape=None
):
    if method == "Conv2DTranspose":
        ...
        x = Conv2D(out_channels * upsampling_factor**2, (3, 3), padding="same")(x)
...
def residual_block(
    ...
    if firstBlock == False:
        shortcut = Conv2D(
            dim, activation=None, kernel_size=(1, 1), strides=first_conv_strides
        )(x)
    else:
        shortcut = Conv2D(dim, activation=None, kernel_size=(1, 1), strides=1)(x)
