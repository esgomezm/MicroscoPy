...
for dataset_name in [
    ...
    train_lr_path = (
        os.path.join(dataset_root, dataset_name, train_lr)
        if train_lr is not None
        else None
    )
    train_hr_path = (
        os.path.join(dataset_root, dataset_name, train_hr)
        if train_hr is not None
        else None
    )
    val_lr_path = (
        os.path.join(dataset_root, dataset_name, val_lr) if val_lr is not None else None
    )
    val_hr_path = (
        os.path.join(dataset_root, dataset_name, val_hr) if val_hr is not None else None
    )
    test_lr_path = (
        os.path.join(dataset_root, dataset_name, test_lr)
        if test_lr is not None
        else None
    )
    test_hr_path = (
        os.path.join(dataset_root, dataset_name, test_hr)
            ...
            for number_of_epochs in [2]:  # [5,10,20]:
                for lr, discriminator_lr in [(0.001, 0.001)]:  # [0.001, 0.005, 0.0005]:
                    ...
                    try:
                        ...
                        print(
                            f"\033[91mERROR\033[0m - In config {dataset_name} {model_name} {number_of_epochs} {lr}"
                        )
                        print(e)
                    gc.collect()
